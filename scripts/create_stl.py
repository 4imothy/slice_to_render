...
def main(folder, output_name):
    ...
    createPointCloud(output_name, points)


if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Must be given folder than output file name as arguments")
        exit(1)
    if len(sys.argv) > 3:
        print("Was given too many arguments, need folder to read and output file")
        exit(1)

    current_file_path = os.path.abspath(__file__)
    folder = os.path.join(os.path.dirname(os.path.dirname(current_file_path)), sys.argv[1])
